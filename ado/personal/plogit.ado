...
program define Estimate, eclass
	...
	* Store unpenalised ML estimates and vector length
	qui logit `yvar' `expltmp' [`weight'`exp'] if `touse', iter(20)
	...
	* Starting values 2
	if missing("`prev'")&missing("`init'") {
		...
		* Create lasso starting values from penalised model
			...
			if ($lambda*`openal'>e(chi2)) {
				...
				qui logit `yvar' [`weight'`exp'] if `touse', iter(20)
			}
			`qui' di as txt "Obtaining starting values:" _c
			`qui' ml model d2 plogi_l1 (`yvar'=`explvar') [`weight'`exp'] if `touse', iter(20) /*
	...
	if missing("`lasso'") {
		...
		* Lasso
		...
		qui logit `yvar' [`weight'`exp'] if `touse', iter(20)
		`qui' ml model d2 plogi_l2 (`yvar'=`explvar') [`weight'`exp'] if `touse', iter(20) /*
			 */ missing continue maximize `init' search(off) nopreserve /*
