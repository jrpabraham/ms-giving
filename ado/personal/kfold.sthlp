...
{p}{cmd:kfold} performs {it:k}-fold cross-validation on a specified model in order to evaluate a model's ability to fit out-of-sample data.{p_end}
...
{p}Finally, {cmd:kfold} reports a measure of goodness-of-fit from each attempt. The default evaluation metric is root mean squared error (RMSE).{p_end}

{title:Syntax}

{cmd:kfold} {it:model} [{it:model_if}] [{it:model_in}] [{it:model_weight}], 
...
{p}{cmd:. kfold reg wage union, mae}
...
{p}{cmd:.kfold reg wage hours grade i.race i.industry i.occupation, r2}
...
{p}{cmd:. kfold qreg wage union [weight=hours], eweight(hours) mae}{p_end}
...
{p}{cmd:. kfold qreg wage union collgrad age grade [weight=hours], eweight(hours) k(3) mae}{p_end}
(importance weights assumed)
...
{p}{cmd:kfold} saves the model errors in the matrix {opt r(stub)} (which is named {bf: r(est)} if no stub name is specified).{p_end}
...
Justin Abraham
jrpabraham@gmail.com

Based on 99% original code from:
