*! version 0.5.1 25sep2010 E. Leuven, B. Sianesi
program define pstest
version 8.0
syntax varlist(min=1) [, Treated(varname) SUPport(varname) MWeight(varname) SUMmary QUIetly]
	...
	breduc `varlist' , mw(`mweight') tr(`treated') sup(`support') `summary' `quietly'
end

program define breduc, rclass
syntax varlist(min=1) , MWeight(varname) TReated(varname) SUPport(varname) [summary quietly]

	tempvar sumbias  sumbias0 weight1

	qui g `weight1' = `mweight' if `treated'==0
	qui replace `weight1' = 1 if `treated'==1
	...
	foreach v of varlist `varlist' {
		...
		qui sum `v' [iw=`weight1'] if `treated'==0 & `support'==1
		scalar `m0m' = r(mean)
		...
		qui regress `v' `treated' [iw=`weight1'] if `support'==1
	...
	if "`summary'"!="" {
		...
		qui probit `treated' `varlist' [iw=`weight1'] if `support'==1
		scalar `r2aft' = e(r2_p)
